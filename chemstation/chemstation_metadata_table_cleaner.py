...
def four_digit_id_to_two_digit(df : pd.DataFrame) -> pd.DataFrame:
    ...
    df['new_id'] = df['exp_id'].astype(str).apply(lambda x : x[1:3] if len(x)==4 else x)
    return df
...
def rename_wine_deg_wines(new_id_series :pd.Series):
    ...
    new_id_series = new_id_series.replace(pattern, lambda x: replacement[x.group(1)], regex=True)
