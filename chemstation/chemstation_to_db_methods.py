...
def write_chemstation_data_to_db_entry(chemstation_data_dicts_tuple, con):
    ...
    chemstation_metadata_list, chromatogram_spectrum_list = chemstation_data_dicts_tuple[0], chemstation_data_dicts_tuple[1]

    if isinstance(chemstation_metadata_list, tuple) & isinstance(chromatogram_spectrum_list, tuple):
        write_chemstation_to_db(chemstation_metadata_list, chemstation_metadata_table_name, chromatogram_spectrum_list, chromatogram_spectrum_table_name, con)
    else:
        print(f'chemstation_metadata_list is dtype {type(chemstation_metadata_list)}, chromatogram_spectrum_list is dtype {type(chromatogram_spectrum_list)}. Both should be list')
        print(f'{chemstation_metadata_list}')
        raise TypeError

def write_chemstation_to_db(chemstation_metadata_list, chemstation_metadata_table_name, chromatogram_spectrum_list, chromatogram_spectrum_table_name, con):
    # write metadata table, chromatogram_spectra table.
    chemstation_metadata_to_db(chemstation_metadata_list, chemstation_metadata_table_name, con)
    chromatogram_spectra_to_db(chromatogram_spectrum_list, chromatogram_spectrum_table_name, con)
    return None
