...
from chemstation_tests import test_logger
...
def create_test_pool(src_dir, dst_parent_dir):
    ...
    """
    # logger.debug(f"sourcing .D from {src_dir}")
    # logger.debug(f"destination path: {dst_parent_dir}")
...
def create_test_data_dir(dst_parent_dir: str, sample_dirpath_list: list):
    ...
    for src_dir in sample_dirpath_list:
        ...
        if os.path.exists(dst_dir):
            test_logger.debug(f"dest already exists: {dst_dir}")
            return
        shutil.copytree(src_dir, dst_dir, dirs_exist_ok=True)

    for dir in os.listdir(dst_parent_dir):
        test_logger.debug(os.path.join(dst_parent_dir, dir))
