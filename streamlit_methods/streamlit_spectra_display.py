...
def spectra_display(df : pd.DataFrame):
    ...
    tab_line_plot, tab_surface_plot, tab_contour_plot = st.tabs(['line plot','surface plot','contour plot'])
...
def main():
    spectra_display(test_spectra_df.test_spectra_df().drop(['name_ct','hash_key'], axis = 1))
