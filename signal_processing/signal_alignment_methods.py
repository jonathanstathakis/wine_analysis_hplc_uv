...
def baseline_subtraction(df_series: pd.Series):
    ...
    for i in baselines.index:
        ...
        baseline_subtracted_y_axis = original_signal_df.iloc[:,1] - baseline_df.iloc[:,1]
        baseline_subtracted_y_axis_colname = f"{list(original_signal_df.columns)[1]}_baseline_subtracted"

        baseline_subtracted_signals[i] = pd.DataFrame()
        baseline_subtracted_signals[i][original_signal_time_axis.name] = original_signal_time_axis
        baseline_subtracted_signals[i][baseline_subtracted_y_axis_colname] = baseline_subtracted_y_axis
...
def interpolate_chromatogram_times(df_series : pd.Series):
    ...
    time_points = np.linspace(start = min_time, stop = max_time, num = df_series.iloc[0][list(df_series.iloc[0].columns)[0]].shape[0])
    ...
    for row_name, row in df_series.items():
        interpolated_chromatogram_df = row.set_index(list(row)[0]).reindex(time_points, method = 'nearest').interpolate().reset_index()
...
def peak_alignment(chromatogram_df_series : pd.Series, highest_corr_key : str):
    ...
    reference_chromatogram_np_array = chromatogram_df_series[highest_corr_key].iloc[1].to_numpy()
