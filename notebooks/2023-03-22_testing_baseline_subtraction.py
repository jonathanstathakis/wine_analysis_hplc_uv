...
signal_1_df = pd.DataFrame(signal_1, columns = ['signal'], index = pd.Index(mins, name = 'mins'))
signal_1_df.name = 'signal_1'

# second signal
signal_2 = rng.standard_normal(501)
signal_2_df = pd.DataFrame(signal_2, columns = ['signal'], index = pd.Index(mins, name = 'mins'))
...
baselines = top_df['signals'].apply(lambda signal : pd.DataFrame(baseline_fitter.iasls(signal['signal'])[0], index = top_df.loc["run_1", "signals"].index, columns = ['baseline'])).to_frame(name = 'baselines')
