...
-  [x] test 1 curve fit:
 ...
 - [x] test 2 reconstruction AUC analysis
       - [x] write skewnorm computer
       - [x] write reconstructor
       - [x] write fit assessor
              - [x] write area computer
...
2024-05-25 00:56:44 - Reconstruction. As already mentioned several times, the reconstruction function simply needs to 1. generate pdfs as a tensor from the computed popt and x.
2024-05-27 09:42:44 - Deconvolution. Now that the fit assessment function is complete, we can move to completing the higher order function. To do this, I will write a test first to execute the full deconvolution function, then compare it to the output of the individually run functions (?)
2024-05-27 10:01:31 - computing bounds. It is interestingly difficult to define the bound computation logic, as it can be dependent on both absolute values, or relative to the initial guess, and is thus dependent on the initial guess calculation. In the interest of permitting modularity, I want to abstract the calculations as much as possible, however, if each value has its own function, we end up with 4 * 3 12 functions dedicated to the calculation, each with their own signatures, and if dependencies exist between them and external parameters, we end up with upwards of 60 or more parameters to manage, and possible tangled dependencies.. especially since we are considering providing several different parameter space computation strategies.. Considering the latter point, best to worry about that later, each can have their own module, just as long as they provide the same interface, then they will be interchangable. Concerning the dependency problem, also best to break the calculation down to primatives then build back up, rather than performing the computations in polars for example, which become hard to modify as they increase in complexity.
2024-05-27 10:25:35 - how to compute bounds. For the initial guesses we need the peak maxima, peak location, peak width. For the bounds we need the initial guesses and the x min and max.
2024-05-27 11:24:57 - TDD flow. A problem with TDD flow is that writing the individual units and testing them before testing the greater function can lead you to waste time on the details, and also miss interface problems. It is very easy to procrastinate the big test. @tdd, @python, @dev