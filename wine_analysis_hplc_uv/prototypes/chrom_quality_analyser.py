...
import pandas as pd
import numpy as np
from agilette.modules.metadata_sampletracker_cellartracker_join import (
    agilent_sample_tracker_cellartracker_super_table as super_table,
)
...
def update_peak_trace(lib, prominence_slider):
    new_peaks = lib.apply(
        lambda row: dt.peak_finder(
            row["nm_254"], 0.05, in_prominence=prominence_slider
        ),
        axis=1,
    )
...
def streamlit_peak_finding(
    signal_df: pd.DataFrame, baseline_df: pd.DataFrame, peak_df: pd.DataFrame
):
    trace_signal = go.Scatter(
        x=signal_df["mins"], y=signal_df["254"], mode="lines", name="signal trace"
    )
    trace_baseline = go.Scatter(
        x=baseline_df[0]["mins"],
        y=baseline_df[0]["mAU"],
        mode="lines",
        name="baseline trace",
    )
    ...
    with st.container():
        prominence_slider = st.slider(
            "select a value for peak prominence", min_value=0, max_value=100
        )
        fig = go.Figure()
...
def main():
    ...
    lib["nm_254"] = pd.Series(
        lib.apply(lambda row: nm_extractor(row["spectrum"], "254"), axis=1)
    )
    lib["nm_254"]
    # create a baseline column.
    # fit the baseline to that wavelength.
    lib["baseline_254"] = pd.Series(
        lib.apply(lambda row: dt.calc_baseline(row["nm_254"]), axis=1)
    )

    # create baseline AUC column
    lib["baseline_254_area"] = pd.Series(
        lib.apply(lambda row: dt.baseline_area(row["baseline_254"]), axis=1)
    )
    ...
    lib["peaks_254"] = lib.apply(
        lambda row: dt.peak_finder(row["nm_254"], 0.05, 2), axis=1
    )
