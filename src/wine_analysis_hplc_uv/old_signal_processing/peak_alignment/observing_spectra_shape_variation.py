...
def report_deviating_shapes(matrix_series: pd.Series):
    ...
    def dim_size_deviation_report(size_series: pd.Series):
        try:
            ...
            deviations = size_series.apply(
                lambda shape: np.abs(np.subtract(shape, dim_mode)).sum()
            )
            ...
            report = pd.DataFrame(
                {
                    ...
                    f"{size_series.name}_%_deviation": round(
                        deviations / dim_mode * 100, 2
                    ),
                }
            )
...
def reshape_dataframe(
    df: pd.DataFrame, desired_rows: int = None, desired_columns: int = None
) -> pd.DataFrame:
    """
