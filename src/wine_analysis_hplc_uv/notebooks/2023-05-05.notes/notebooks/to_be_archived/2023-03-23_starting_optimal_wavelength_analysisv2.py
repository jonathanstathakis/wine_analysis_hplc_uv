...
def main():
    ...
    sample_uv_paths = [
        "/Users/jonathan/0_jono_data/2023-02-15_COFFEE_COLUMN_CHECK.D/DAD1.UV",
        "/Users/jonathan/0_jono_data/5test_sequence/2023-02-22_STONEY-RISE-PN_02-21.D/DAD1.UV",
    ]

    # form a df with the list of files as the first column
    uv_dataframe = pd.DataFrame(
        sample_uv_paths,
        columns=["file_path"],
        index=pd.Index(
            [Path(path).parent.name for path in sample_uv_paths], name="run"
        ),
    )

    uv_dataframe["uv_object"] = uv_dataframe.apply(
        lambda row: rb.agilent.chemstation.parse_uv(row["file_path"]), axis=1
    )
    ...
    uv_dataframe["spectrum"] = uv_dataframe.apply(
        lambda row: pd.DataFrame(
            row["uv_object"].data,
            columns=pd.Index(row["uv_object"].ylabels, name="nm"),
            index=pd.Index(row["uv_object"].xlabels, name="mins"),
        ),
        axis=1,
    )
    ...
    uv_dataframe["scaled_spectrum"] = uv_dataframe["spectrum"].apply(
        lambda col: pd.DataFrame(
            scaler.fit_transform(col), columns=col.columns, index=col.index
        )
    )
    ...
    uv_dataframe["baseline_obj"] = uv_dataframe["scaled_spectrum"].apply(
        lambda col_spectrum: Baseline(col_spectrum.index)
    )

    uv_dataframe["spectrum_baseline"] = uv_dataframe.apply(
        lambda row: row["scaled_spectrum"].apply(
            lambda spectrum_col: row["baseline_obj"].iasls(spectrum_col)[0]
        ),
        axis=1,
    )
    ...
    uv_dataframe["baseline_corrected_scaled_spectrum"] = uv_dataframe.apply(
    ...
    uv_dataframe["av_baseline_grad"] = uv_dataframe.apply(
        lambda row: pd.DataFrame(
            row["spectrum_baseline"].apply(
                lambda baseline_col: np.mean(np.gradient(baseline_col))
            ),
            columns=["av_baseline_gradient"],
        ),
        axis=1,
    )
    ...
    def peak_getter(nm_col):
        ...
        series = pd.Series(
            peak_y, index=pd.Index(peak_x, name="mins"), name=f"{nm_col.name}_peaks"
        )
    ...
    uv_dataframe["peaks_per_nm"] = uv_dataframe.apply(
        lambda row: row["baseline_corrected_scaled_spectrum"].apply(
            lambda col: peak_getter(col)
        ),
        axis=1,
    )
    ...
    peak_avs = uv_dataframe.apply(
        lambda row: row["peaks_per_nm"].apply(lambda col: np.mean(col)),
        axis=1,
        result_type="reduce",
    )
