...
def uv_data_scaler(uv_data_column):
    ...
    def scale_spectrum(spectrum):
        ...
        try:
            scaled_spectrum = pd.DataFrame(
                scaler.fit_transform(spectrum), columns=spectrum.columns
            )
...
def spectrum_baseline_calculation(spectrum_column):
    ...
    if isinstance(return_spectrum_column, pd.Series):
        ...
        print(
            f"return_spectrum_column is a {type(return_spectrum_column)}, not a Series"
        )
