...
def extract_vintage_from_name(df):
    wines_without_vintage_mask = (
        (df["vintage"].isna())
        & ~(df["name"].isna())
        & ~(df["name"] == "missing vials")
        & ~(df["name"] == "mystery")
    )
    ...
    wines_without_vintage["vintage"] = wines_without_vintage["name"].str.extract(
        match_two_digit_vintage_string, expand=False
    )

    # remove the residual vintages from name.

    wines_without_vintage["name"] = wines_without_vintage["name"].str.replace(
        match_two_digit_vintage_string, "", regex=True
    )

    # strip residual whitespace

    wines_without_vintage["name"] = wines_without_vintage["name"].apply(
        lambda x: x.strip()
    )

    # format two digit vintages as 4 digits by adding '20'. Note: be aware of any 20th century wines further down the track..

    wines_without_vintage["vintage"] = wines_without_vintage["vintage"].apply(
        lambda x: f"20{x}"
    )
...
def main():
    ...
    df["name"] = df["name"].replace(
        {" ch$": " chardonnay", " shz$": " shiraz", "pn$": "pinot noir"}, regex=True
    )
    ...
    response = post_df_as_sheet_values(
        df, in_sheet, range, creds_parent_path=creds_path
    )
